...
class GroupGamingAnalyzer {
    ...
    async analyzeGroupCompatibility(steamIds) {
        const playersData = await Promise.all(steamIds.map(async (steamId) => {
    ...
    async findCommonGames(steamIds, minOwnedBy = 2) {
        const playersGames = await Promise.all(steamIds.map(steamId => this.steamClient.getOwnedGames(steamId)));
    ...
    async generateGroupRecommendations(steamIds, groupSize, maxRecommendations = 10) {
        const playersData = await Promise.all(steamIds.map(async (steamId) => {
            const [player, games] = await Promise.all([
                this.steamClient.getPlayerSummary(steamId),
                this.steamClient.getOwnedGames(steamId)
    ...
    analyzeGroupProfile(playersData) {
    ...
    calculateCompatibilityScore(playersData) {
    ...
    analyzeGroupDynamics(playersData) {
        const experienceLevels = playersData.map(p => {
            const totalHours = p.games.reduce((sum, game) => sum + game.playtime_forever, 0) / 60;
            if (totalHours < 100)
                return 'casual';
            if (totalHours < 1000)
                return 'regular';
        ...
        let groupType = 'mixed';
        if (casualCount === playersData.length)
            groupType = 'casual';
        else if (hardcoreCount === playersData.length)
            groupType = 'hardcore';
        else if (regularCount >= playersData.length / 2)
            groupType = 'regular';
    ...
    generateRecommendations(playersData, groupAnalysis, maxRecommendations) {
    ...
    determineGroupDynamics(playersData) {
        const avgHours = playersData.reduce((sum, p) => sum + p.analysis.totalHours, 0) / playersData.length;
        if (avgHours < 500)
            return 'Casual Gaming Group';
        if (avgHours < 2000)
            return 'Regular Gaming Group';
        return 'Hardcore Gaming Group';
    }
    getRecommendedGenres(groupType) {
        switch (groupType) {
    ...
    isMultiplayerGame(gameName) {
        ...
        return multiplayerKeywords.some(keyword => gameName.toLowerCase().includes(keyword));
    }
}
...
function validateGroupCompatibilityArgs(args) {
    if (!args || typeof args !== 'object') {
        throw new McpError(ErrorCode.InvalidParams, 'Arguments must be an object');
    }
    const argsObj = args;
...
function validateGroupRecommendationsArgs(args) {
    if (!args || typeof args !== 'object') {
        throw new McpError(ErrorCode.InvalidParams, 'Arguments must be an object');
    }
    const argsObj = args;
    ...
    return {
        ...
        priceRange: typeof argsObj.priceRange === 'string' ? argsObj.priceRange : 'any'
    };
}
...
class SteamMCPServer {
            ...
            try {
                switch (name) {
                    ...
                    case 'analyze_group_compatibility':
                        ...
                        return await this.handleAnalyzeGroupCompatibility(compatibilityArgs.steamIds, compatibilityArgs.includeDetails);
                    case 'find_common_games':
                        const commonGamesArgs = validateFindCommonGamesArgs(args);
                        return await this.handleFindCommonGames(commonGamesArgs.steamIds, commonGamesArgs.minOwnedBy);
                    case 'generate_group_recommendations':
                        const recommendationsArgs = validateGroupRecommendationsArgs(args);
                        return await this.handleGenerateGroupRecommendations(recommendationsArgs.steamIds, recommendationsArgs.maxRecommendations, recommendationsArgs.priceRange, recommendationsArgs.includeOwned);
                    default:
    ...
    async handleAnalyzeGroupCompatibility(steamIds, includeDetails) {
    ...
    async handleFindCommonGames(steamIds, minOwnedBy) {
    ...
    async handleGenerateGroupRecommendations(steamIds, maxRecommendations, priceRange, includeOwned) {
        const recommendations = await this.groupAnalyzer.generateGroupRecommendations(steamIds, steamIds.length, maxRecommendations);
    ...
    generatePlayNowRecommendations(commonGames) {
        const recommendations = [];
        ...
        const coopGames = commonGames.filter(game => game.name.toLowerCase().includes('co-op') ||
    ...
    identifyPlayNowOptions(recommendations) {
        return recommendations
            .filter(game => {
    ...
    filterByPriceRange(recommendations, priceRange) {
        const priceFilters = {
            ...
            under20: (price) => price < 20,
            under50: (price) => price < 50,
            any: () => true
        };
        const filter = priceFilters[priceRange] || priceFilters.any;
        return recommendations.filter(game => filter(game.price));
    }
    generateRecommendationSummary(recommendations, groupAnalysis) {
        const insights = [];
